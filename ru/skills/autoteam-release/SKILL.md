...
**Вход.** Поручение выкатить / инцидент / готовый срез изменений; механизм деплоя проекта. **Выход.** Выкат со smoke-evidence, запись в runbook при инциденте. **Запреты.** Wipe, force-все, тяжёлые операции под нагрузкой, commit-push «заодно», выдуманный scope, чужие хосты; разрушительные git-операции в локальном дереве разработчика (`reset --hard`, `clean`); выкатка с фиче-веток — срез идёт из интеграционной ветки. **Evidence.** Smoke-команда + вывод (или проверка URL после деплоя платформой). **Стоп.** Нет «выкати/можно» — срез готов и один вопрос, не зависай. Объявление production — человек. **Дальше.** Инцидент с границей данных → очередь `autoteam-architecture-review`. Чужой scope не стопай.
