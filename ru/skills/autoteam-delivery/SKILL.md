...
Стек: `docs/process/stack.md`. Ремесло: `docs/delivery/README.md` канона метода — qa.md всегда; стековое ремесло (правила конкретного языка/фреймворка) — overlay продукта, не канон.
...
Чужой тест не изменяется и не ослабляется тихо: отдельным разделом артефакта — какое свойство тест охранял, почему оно перестало быть актуальным, какая альтернатива отвергнута. Без этого раздела приёмка обязана отклонить.
