...
0. Ретро — задача **над траекторией**: читает журнал, handoff и чекпойнты — одного состояния не хватает по построению, потому что состояние намеренно держит только нужное следующей волне. Ради этого записи журнала и не переписываются.
