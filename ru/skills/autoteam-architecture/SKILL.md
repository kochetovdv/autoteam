...
Компоненты и владельцы данных → контракты до кода → отказы (таймаут, повтор, идемпотентность) → нагрузка цифрами или ASSUMP → права и карта приватности (режим по юрисдикции) → наблюдаемость → выкатка/откат → ADR.
