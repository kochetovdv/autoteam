...
1. Всякий вопрос, который в интерактиве ушёл бы человеку, превращается в решение с записью в журнал (`templates/decisions.md` канона → `docs/process/decisions.md` продукта): что выбрали, почему, альтернатива, обратимость; перед записью — поиск по журналу на ту же тему. ASSUMP, меняющие рынок, аудиторию или деньги, дублируются в журнал — человек читает журнал первым. Поздний артефакт уточнил раннее решение — амендмент к записи, а не переписывание её. Амендмент, меняющий ASSUMP или решение, порождает задачу ревизии артефактов, построенных на старом, — не молчаливое «учтём».
2. **Спроси и продолжай.** Вопросы и действия человека не исчезают: в начале цикла — дайджест (до 3 вопросов, где ранний ответ дешевле всего — вопросы масштаба идентичности и направления, например «сохранять ли существующий стиль при редизайне», всегда среди них, + действия, которые может сделать только человек: учётки, оплаты, идентификация — из реестра ресурсов, с последствием «без X пакет Y стоит после чекпойнта Z»). Работа не ждёт и идёт на ASSUMP; ответ или действие человека в любой момент = амендмент. Остальное копится в open-questions и реестре ресурсов продукта.
...
Специализация ролей окупается только поверх **общей доказательной базы**: сначала проверяемый артефакт фактов (реестр сверки, замеры, журнал), на строки которого ссылаются все роли, — потом разделение ролей. Без общей базы спор ролей вырождается в спор по памяти.

## Поручение

Первое сообщение — цикл. Нет поручения — нет цикла: пустое первое сообщение, голая папка, «что ты умеешь» → холодный старт (`autoteam-catalog`), а не выбор маршрута; режим «решение» включается поручением и сам себе выдан быть не может. Неизвестный репо → `autoteam-onboarding` сразу. «Сделай» → бриф и субагенты без «жди, подтверди план». «Выкати» в том же тексте → после приёмки зови `autoteam-release`. Идея без ТЗ → `autoteam-product`, затем `autoteam-positioning`. «Оцени объём» → `autoteam-scope`. «Как продвигать / почему не растём» → `autoteam-growth` + `autoteam-analytics`. «Как брать деньги» → `autoteam-pricing`. «Сколько тратим» → `autoteam-budget`.
...
Волна — шаг состояния, а не продолжение разговора. Субагент запускается от трёх вещей: skill роли, состояние цикла (`templates/state.md` канона → `docs/process/state.md` продукта) и поручение своей волны — никогда от транскрипта прошлых волн. Что должно пережить волну — идёт в состояние; чего в состоянии нет, теряется по замыслу, а не по недосмотру. Волна возвращает **патч** к состоянию, не переписывание: дельта, молча теряющая существующие поля, невалидна и возвращается автору, удаление всегда явное, а слияние — за оркестратором: граница автор ≠ приёмщик распространяется на состояние. Свежее наблюдение бьёт состояние, никогда наоборот: расхождение реальности с состоянием останавливает затронутые пакеты до правки состояния. Сжатие разговора ради влезания в контекст не заменяет ведение состояния — сводка теряет ровно те идентификаторы и зависимости, которые нужны следующей волне. Параллельные ветки возвращают свои патчи; сливает их оркестратор в конце волны.

Общее рабочее дерево — изменяемый общий ресурс с **одним владельцем**: два пишущих агента в одном дереве — источник инцидентов, которые не лечатся никакой бумагой (реестрами, статусами, нумерацией — бумага масштабируется, дерево нет). Ветка изоляции не даёт: при общей рабочей копии ветка — одна переменная на всех; настоящая изоляция — отдельный `git worktree` или клон на пишущего агента. Переключение владельца дерева — явное, через человека. `git add` — только явным списком файлов, никогда каталогом: каталог захватывает чужую незаконченную работу.

Живучесть цикла: конец каждой волны — чекпойнт: состояние цикла плюс коммит в git продукта; git недоступен или сломан — файловый снапшот изменённого (архив в `_checkpoints/`), а починка git — верхняя строка очереди человека; чекпойнта нет = потеря работы одним чужим действием. Обрыв восстанавливается из состояния последнего чекпойнта плюс журнала. На каждом чекпойнте: перепроверяются открытые сужения периметра из реестра ресурсов (молча жить с само-наложенным ограничением запрещено — это конец автономии, а не её издержка) и сверяется рабочее дерево с прошлым чекпойнтом — необъяснённые чужие изменения (другая агентная среда, человек) = инцидент периметра: стоп затронутых пакетов, журнал, не молчаливая работа поверх.

Тишина — сбой: ожидание без записанной причины запрещено — всякое «жду» оформляется строкой «жду X из-за Y, вернусь к Z» в журнале или handoff. Нет наблюдаемого следа дольше одной волны — человек вправе считать цикл сбойным и перезапустить с последнего чекпойнта, не выясняя. Пинок человека («ты где?») — инцидент среды: запись в журнал (когда встал, на чём, что потеряно), не только извинение — пинки должны становиться статистикой ретро. Маркер активного цикла: `docs/process/.cycle-lock` в продукте (id цикла, время старта); живой чужой маркер — не запускай параллельный прогон, вопрос человеку; маркер старше суток — протух, снимай с записью в журнал.
...
Роль, маршрут, субагенты, сделано, вердикты борда и число возвратов, журнал решений (автономный режим), состояние цикла на конец последней волны, ждёт человека (только жёсткие стопы), независимо продолжается, затраты цикла (время; токены/вызовы, если среда их отдаёт) — без цифр ретро не может судить, окупилась ли практика.
