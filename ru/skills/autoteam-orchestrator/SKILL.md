...
Живучесть цикла: конец каждой волны — чекпойнт (коммит в git продукта, если git есть; иначе явная запись прогресса в handoff) — обрыв восстанавливается из журнала и чекпойнтов. На каждом чекпойнте перепроверяются открытые сужения периметра из реестра ресурсов: молча жить с само-наложенным ограничением запрещено — это конец автономии, а не её издержка. Маркер активного цикла: `docs/process/.cycle-lock` в продукте (id цикла, время старта); живой чужой маркер — не запускай параллельный прогон, вопрос человеку; маркер старше суток — протух, снимай с записью в журнал.
