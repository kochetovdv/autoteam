...
2. **Спроси и продолжай.** Вопросы человеку не исчезают: в начале цикла — дайджест до 3 вопросов, где ранний ответ дешевле всего; работа не ждёт и идёт на ASSUMP; ответ человека в любой момент = амендмент. Остальные вопросы копятся в open-questions продукта.
3. Жёстких стопов три, и только они: внешние деньги (оплата, публикация цен, платный бюджет); право и публичные обязательства от имени человека; необратимое разрушение (wipe данных, удаление чужого, прод без «выкати»). Всё остальное — решение, не вопрос.
...
Финальная приёмка решения — **панель по призмам**, не один критик:

- **коммерческий**: продаёт ли — оффер, посылы, доверие, путь к деньгам;
- **инженерный**: код, данные, безопасность, отказы, evidence проверок;
- **арт**: планка жанра, типографика, motion, мобильный UX (скриншоты обеих версий обязательны);
- профильные по риску: security (деньги/ПДн/роли), quality (гонки/данные).

Каждый критик — отдельный субагент со свежим контекстом и мандатом «искать причины отклонить»; свой вердикт `ACCEPTED` | `CHANGES_REQUIRED`. Любой `CHANGES_REQUIRED` — возврат исполнителю; до 2 циклов, затем фиксация в журнале и упрощение решения. Возвраты — норма процесса, не сбой: их число и замечания фиксируются в handoff (статистика «0 возвратов всегда» — повод проверить мягкость борда на ретро).
